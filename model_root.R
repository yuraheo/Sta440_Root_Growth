model {
  for (i in 1:N) {
    ...
    # Assume wildtype inner = mutant inner
    xmid_eff[i] <- alpha[root[i]] +
      gamma1 * side[i] +                # WT outer vs inner
      gamma2 * side[i] * geno[i]        # extra outer shift for mutant
  }
  ...
  # Midpoint mean around 0.5 on the logit scale (logit(0.5) = 0)
  # Precision = 1  -> sd = 1  (midpoints mostly within ~0.1–0.9)
  mu_z0 ~ dnorm(0, 1)
  ...
  # between-root variation (fairly vague)
  prec_a      ~ dgamma(1, 1)
  prec_d      ~ dgamma(1, 1)
  prec_z0     ~ dgamma(1, 1)
  ...
  # Half-t(df = 3, scale = 3) prior for sigma
  sigma <- abs(t_sigma)
  t_sigma ~ dt(0, pow(3, -2), 3)   # mean 0, scale = 3, df = 3
}